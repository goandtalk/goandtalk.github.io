...
var _ = self.Mavo = $.Class({
	...
	destroy: function(){
		...
		this.backendObserver && this.backendObserver.destroy();
		this.modeObserver && this.modeObserver.destroy();
...
var _ = Mavo.UI.Bar = $.Class({
	constructor: function(mavo) {
		...
		this.controls.forEach(id => {
			...
			this[id].setAttribute('mv-ownerapp', this.mavo.id);
...
var _ = Mavo.Primitive = $.Class({
	...
	constructor: function (element, mavo, o) {
		...
		this.element.setAttribute('mv-ownerapp', (this.mavo || mavo).id);
	},
	...
	initEdit: function () {
		...
		this.editor.setAttribute('mv-ownerapp', this.mavo.id);
		$.bind(this.editor, {
			"input change": evt => {
...
var _ = Mavo.Expressions = $.Class({
	...
	register: function(domexpression) {
		...
		//goandmake code, avoid recursive if there are more than 1 external app
